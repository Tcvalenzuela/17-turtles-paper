...
#    print (line)
    ...
    gffEnd=int(Split[4])
 #   print (str(gffScaff)+"\t"+str(Start)+"\t"+str(gffEnd)+"\t"+str(gffclass))
  #  print (line)
    if gffclass == "region":
        continue
    elif gffclass == "exon":
...
#        print (ele+"\t"+rec)
        if Gffgene[ele][2]!= TEStarts[rec][1]:
            ...
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "gene" + "_inside_TE"+"\t" +ele )
            TEsinGENES[rec]=("gene_inside_TE")
        elif Gffgene[ele][0] < TEStarts[rec][0] and Gffgene[ele][0] < TEEnds[rec][0] and  Gffgene[ele][1] > TEEnds[rec][0] and TEStarts[rec][0] < Gffgene[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "\t"+"TE_inside_"+ "gene" +"\t"+ ele)
            TEsinGENES[rec]=("TE_inside_gene")
        elif Gffgene[ele][0] <= TEStarts[rec][0] and Gffgene[ele][0] < TEEnds[rec][0] and  Gffgene[ele][1] <= TEEnds[rec][0] and TEStarts[rec][0] < Gffgene[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2Downstream_"+ "gene" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2Downstream_gene")
        elif Gffgene[ele][0] >= TEStarts[rec][0] and Gffgene[ele][0] < TEEnds[rec][0] and  Gffgene[ele][1] >= TEEnds[rec][0] and TEStarts[rec][0] < Gffgene[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2upstream_"+"gene" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2upstream_gene")
        elif Gffgene[ele][0] == TEStarts[rec][0] and Gffgene[ele][0] < TEEnds[rec][0] and  Gffgene[ele][1] == TEEnds[rec][0] and TEStarts[rec][0] < Gffgene[ele][1] :
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_exactly_"+ "gene"+"\t" + ele)
...
for ele in GffmRNA:
    for rec in TEStarts:
        if GffmRNA[ele][2]!= TEStarts[rec][1]:
            ...
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "mRNA" + "_inside_TE"+"\t" +ele)
            TEsinGENES[rec]=("mRNA_inside_TE")
        elif GffmRNA[ele][0] < TEStarts[rec][0] and GffmRNA[ele][0] < TEEnds[rec][0] and  GffmRNA[ele][1] > TEEnds[rec][0] and TEStarts[rec][0] < GffmRNA[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "\t"+"TE_inside_"+ "mRNA" +"\t"+ ele)
            TEsinGENES[rec]=("TE_inside_mRNA")
        elif GffmRNA[ele][0] <= TEStarts[rec][0] and GffmRNA[ele][0] < TEEnds[rec][0] and  GffmRNA[ele][1] <= TEEnds[rec][0] and TEStarts[rec][0] < GffmRNA[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2Downstream_"+ "mRNA" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2Downstream_mRNA")
        elif GffmRNA[ele][0] >= TEStarts[rec][0] and GffmRNA[ele][0] < TEEnds[rec][0] and  GffmRNA[ele][1] >= TEEnds[rec][0] and TEStarts[rec][0] < GffmRNA[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2upstream_"+"mRNA" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2upstream_mRNA")
        elif GffmRNA[ele][0] == TEStarts[rec][0] and GffmRNA[ele][0] < TEEnds[rec][0] and  GffmRNA[ele][1] == TEEnds[rec][0] and TEStarts[rec][0] < GffmRNA[ele][1] :
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_exactly_"+ "mRNA"+"\t" + ele)
...
for ele in Gffexon:
    for rec in TEStarts:
        if Gffexon[ele][2]!= TEStarts[rec][1]:
            ...
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "exon" + "_inside_TE"+"\t" +ele)
            TEsinGENES[rec]=("exon_inside_TE")
        elif Gffexon[ele][0] < TEStarts[rec][0] and Gffexon[ele][0] < TEEnds[rec][0] and  Gffexon[ele][1] > TEEnds[rec][0] and TEStarts[rec][0] < Gffexon[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "\t"+"TE_inside_"+ "exon" +"\t"+ ele)
            TEsinGENES[rec]=("TE_inside_exon")
        elif Gffexon[ele][0] <= TEStarts[rec][0] and Gffexon[ele][0] < TEEnds[rec][0] and  Gffexon[ele][1] <= TEEnds[rec][0] and TEStarts[rec][0] < Gffexon[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2Downstream_"+ "exon" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2Downstream_exon")
        elif Gffexon[ele][0] >= TEStarts[rec][0] and Gffexon[ele][0] < TEEnds[rec][0] and  Gffexon[ele][1] >= TEEnds[rec][0] and TEStarts[rec][0] < Gffexon[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2upstream_"+"exon" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2upstream_exon")
        elif Gffexon[ele][0] == TEStarts[rec][0] and Gffexon[ele][0] < TEEnds[rec][0] and  Gffexon[ele][1] == TEEnds[rec][0] and TEStarts[rec][0] < Gffexon[ele][1] :
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_exactly_"+ "exon"+"\t" + ele)
...
for ele in GffCDS:
    for rec in TEStarts:
        if GffCDS[ele][2]!= TEStarts[rec][1]:
            ...
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "CDS" + "_inside_TE"+"\t" +ele)
            TEsinGENES[rec]=("CDS_inside_TE")
        elif GffCDS[ele][0] < TEStarts[rec][0] and GffCDS[ele][0] < TEEnds[rec][0] and  GffCDS[ele][1] > TEEnds[rec][0] and TEStarts[rec][0] < GffCDS[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "\t"+"TE_inside_"+ "CDS" +"\t"+ ele)
            TEsinGENES[rec]=("TE_inside_CDS")	
        elif GffCDS[ele][0] <= TEStarts[rec][0] and GffCDS[ele][0] < TEEnds[rec][0] and  GffCDS[ele][1] <= TEEnds[rec][0] and TEStarts[rec][0] < GffCDS[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2Downstream_"+ "CDS" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2Downstream_CDS")
        elif GffCDS[ele][0] >= TEStarts[rec][0] and GffCDS[ele][0] < TEEnds[rec][0] and  GffCDS[ele][1] >= TEEnds[rec][0] and TEStarts[rec][0] < GffCDS[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2upstream_"+"CDS" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2upstream_CDS")
        elif GffCDS[ele][0] == TEStarts[rec][0] and GffCDS[ele][0] < TEEnds[rec][0] and  GffCDS[ele][1] == TEEnds[rec][0] and TEStarts[rec][0] < GffCDS[ele][1] :
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_exactly_"+ "CDS"+"\t" + ele)
...
for ele in Gfftranscript:
    for rec in TEStarts:
        if Gfftranscript[ele][2]!= TEStarts[rec][1]:
            ...
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "transcript" + "_inside_TE"+"\t" +ele)
            TEsinGENES[rec]=("transcript_inside_TE")
        elif Gfftranscript[ele][0] < TEStarts[rec][0] and Gfftranscript[ele][0] < TEEnds[rec][0] and  Gfftranscript[ele][1] > TEEnds[rec][0] and TEStarts[rec][0] < Gfftranscript[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "\t"+"TE_inside_"+ "transcript" +"\t"+ ele)
            TEsinGENES[rec]=("TE_inside_transcript")
        elif Gfftranscript[ele][0] <= TEStarts[rec][0] and Gfftranscript[ele][0] < TEEnds[rec][0] and  Gfftranscript[ele][1] <= TEEnds[rec][0] and TEStarts[rec][0] < Gfftranscript[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2Downstream_"+ "transcript" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2Downstream_transcript")
        elif Gfftranscript[ele][0] >= TEStarts[rec][0] and Gfftranscript[ele][0] < TEEnds[rec][0] and  Gfftranscript[ele][1] >= TEEnds[rec][0] and TEStarts[rec][0] < Gfftranscript[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2upstream_"+"transcript" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2upstream_transcript")
        elif Gfftranscript[ele][0] == TEStarts[rec][0] and Gfftranscript[ele][0] < TEEnds[rec][0] and  Gfftranscript[ele][1] == TEEnds[rec][0] and TEStarts[rec][0] < Gfftranscript[ele][1] :
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_exactly_"+ "transcript"+"\t" + ele)
...
for ele in GffcDNA_match:
    for rec in TEStarts:
        if GffcDNA_match[ele][2]!= TEStarts[rec][1]:
            ...
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "cDNA_match" + "_inside_TE"+"\t" +ele)
            TEsinGENES[rec]=("cDNA_match_inside_TE")
        elif GffcDNA_match[ele][0] < TEStarts[rec][0] and GffcDNA_match[ele][0] < TEEnds[rec][0] and  GffcDNA_match[ele][1] > TEEnds[rec][0] and TEStarts[rec][0] < GffcDNA_match[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "\t"+"TE_inside_"+ "cDNA_match" +"\t"+ ele)
            TEsinGENES[rec]=("TE_inside_cDNA_match")
        elif GffcDNA_match[ele][0] <= TEStarts[rec][0] and GffcDNA_match[ele][0] < TEEnds[rec][0] and  GffcDNA_match[ele][1] <= TEEnds[rec][0] and TEStarts[rec][0] < GffcDNA_match[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2Downstream_"+ "cDNA_match" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2Downstream_cDNA_match")
        elif GffcDNA_match[ele][0] >= TEStarts[rec][0] and GffcDNA_match[ele][0] < TEEnds[rec][0] and  GffcDNA_match[ele][1] >= TEEnds[rec][0] and TEStarts[rec][0] < GffcDNA_match[ele][1]:
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_fromInside2upstream_"+"cDNA_match" +"\t"+ ele)
            TEsinGENES[rec]=("TE_fromInside2upstream_cDNA_match")
        elif GffcDNA_match[ele][0] == TEStarts[rec][0] and GffcDNA_match[ele][0] < TEEnds[rec][0] and  GffcDNA_match[ele][1] == TEEnds[rec][0] and TEStarts[rec][0] < GffcDNA_match[ele][1] :
            print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) +"\t"+ "TE_exactly_"+ "cDNA_match"+"\t" + ele)
            TEsinGENES[rec]=("TE_exactly_cDNA_match")

for rec in TEStarts:
	if rec not in TEsinGENES:
		print (rec.rstrip("\n")+"\t"+str(TEsize[TE][0]) + "TE_Not_in_genes"+ "-	-	-	-	-	-	-	-	-")
